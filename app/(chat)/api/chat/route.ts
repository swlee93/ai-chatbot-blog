...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (messageCount >= entitlementsByUserType[userType].maxMessagesPerDay) {
      ...
      const rateLimitMessage = `Daily message limit reached. Limits: guest ${guestLimit}/day, regular ${regularLimit}/day.`;
      return new ChatSDKError("rate_limit:chat", undefined, rateLimitMessage).toResponse();
    }
    ...
    // Load blog context before creating stream
    let blogContext: string | undefined;
    let blogSources: Array<{filePath: string; title?: string; section?: string}> | undefined;
    const userMessage = message?.parts.find(p => p.type === 'text');
    if (userMessage && 'text' in userMessage) {
      const { getSmartContext } = await import('@/lib/blog/context-loader');
      
      // Load context for ALL queries - blog is always available
      const { context, sources } = await getSmartContext(userMessage.text);
      blogContext = context;
      blogSources = sources;
      ...
      onFinish: async ({ messages: finishedMessages }) => {
        ...
        if (blogSources && blogSources.length > 0) {
          ...
          if (assistantMessage) {
            ...
            console.log('⚠️ No assistant message found in finished messages');
          }
        } else {
          console.log('⚠️ No blog sources to save');
        }
      },
      onError: () => {
